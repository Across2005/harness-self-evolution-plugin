 ...
 */
export async function loadCompleteHistory({ read, loadOlder, signal, maxPages = 1_000, }) {
    let pages = 0;
    while (pages < maxPages) {
        if (signal.aborted)
            return { kind: 'cancelled', pages };
        const before = read();
        if (!before.hasMore)
            return { kind: 'complete', pages };
        if (before.loadingOlder)
            return { kind: 'blocked', pages, reason: 'busy' };
        await loadOlder();
        if (signal.aborted)
            return { kind: 'cancelled', pages };
        const after = read();
        if (!after.hasMore)
            return { kind: 'complete', pages: pages + 1 };
