export const OVERVIEW_STATE_LABEL = Object.freeze({
    active: '进行中',
    current: '当前',
    waiting: '等待你',
    failure: '有失败记录',
...
export function overviewStateOf(status, isLatest) {
    if (status === 'waiting')
        return 'waiting';
    if (status === 'failure')
        return 'failure';
...
export function groupOverviewSummary(group) {
    return group.executionCount > 1 ? `${group.executionCount} 次执行` : '';
}
export function turnOverviewSummary(turn) {
...
export function turnNeedsDefaultDisclosure(state, isLatest) {
