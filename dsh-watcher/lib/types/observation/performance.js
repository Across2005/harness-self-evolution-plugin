function outputTokensOf(usage) {
    if (typeof usage !== 'object' || usage === null || !('outputTokens' in usage))
        return null;
...
function toolDurationOf(turn) {
    ...
    for (const group of turn.groups) {
        for (const item of group.items) {
            if (item.source !== 'tool' || item.durationMs === null)
                continue;
 ...
 */
export function deriveTurnPerformance(nodes, turns) {
    const assistantFolds = foldAssistantNodes(nodes);
    const performance = new Map();
    for (const turn of turns) {
        const fold = assistantFolds.get(turn.turn);
        const throughput = fold !== undefined && fold.decodeSampled && fold.decodeMs > 0
            ? {
                kind: 'measured',
...
export function turnElapsedReading(turn, live, now) {
...
export function turnElapsedMs(turn, live, now) {
...
export function groupElapsedMs(group, live, now) {
 ...
 */
export function deriveSessionTiming(picture, now) {
