import { foldModelTraceEvents, hasReasoningEvidence } from "./model-trace.js";
const EMPTY_NOW = Object.freeze({ phase: 'other', label: '', status: 'unknown' });
export const EMPTY_PICTURE = Object.freeze({
    nodes: [],
    turns: [],
    now: EMPTY_NOW,
    actionCount: 0,
...
export function normalizedSignature(toolName, args) {
...
function textFromContent(value, output = []) {
    ...
    if (Array.isArray(value.content))
        textFromContent(value.content, output);
    if (isRecord(value.message))
        textFromContent(value.message, output);
    return output;
}
...
function findBooleanByKeys(value, keys, depth = 0) {
    ...
    if (!isRecord(value))
        return null;
    for (const [key, child] of Object.entries(value)) {
        if (keys.has(key) && typeof child === 'boolean')
            return child;
    }
    for (const child of Object.values(value)) {
        const found = findBooleanByKeys(child, keys, depth + 1);
...
function domainOutcomeOf(...values) {
    for (const value of values) {
        ...
        if (status !== undefined && status !== null) {
            if (/^(ok|success|succeeded|passed|done|completed)$/.test(status))
                return 'success';
            if (/^(error|failed|failure|denied|cancelled|canceled)$/.test(status))
                return 'failure';
        }
    }
    return null;
...
function phaseOfTool(name, args, callView) {
    ...
    if (/^(skill|todo_write|create_goal|update_goal|get_goal|update_plan)$/.test(name))
        return 'plan';
    if (/^(read|grep|glob|find|search|web_search|web_fetch|web_open)$/.test(name))
        return 'investigate';
    if (/^(write|edit|apply_patch|imagegen|image_gen)/.test(name))
        return 'build';
    if (/^(dshx_activation_plan|dshx_sync_artifact|dshx_ship|dshx_install|dshx_start)/.test(name))
        return 'activate';
    if (/^(dshx_check|dshx_verify|dshx_which|dshx_status)/.test(name))
        return 'verify';
    if (name.startsWith('cua_') || name.includes('computer'))
        return 'desktop';
    if (name === 'bash') {
        const command = commandOf(args);
        ...
        if (isVerificationCommand(command))
            return 'verify';
        if (/\b(?:sed|head|tail|ls|find|rg|grep|git\s+(?:status|diff|log|show))\b/.test(command))
            return 'investigate';
        return 'build';
    }
...
function titleOfTool(name, args, callView) {
    ...
    if (name === 'read')
        return `读取 ${target === null ? '文件' : baseName(target)}`;
    if (name === 'grep')
        return `搜索 ${stringArg(args, 'pattern') ?? '内容'}`;
    if (name === 'glob')
        return `查找 ${stringArg(args, 'pattern') ?? '文件'}`;
    if (name === 'write')
        return `写入 ${target === null ? '文件' : baseName(target)}`;
    if (name === 'edit')
        return `修改 ${target === null ? '文件' : baseName(target)}`;
    if (name === 'apply_patch')
        return '应用代码补丁';
    if (name === 'bash') {
    ...
    if (name === 'skill')
        return '读取工作说明';
    if (name === 'todo_write' || name === 'update_plan')
        return '更新工作计划';
    if (name === 'create_goal')
        return '建立任务目标';
    if (name === 'update_goal')
    ...
    if (name === 'get_goal')
        return '核对任务目标';
    if (name === 'dshx_status' || name === 'dshx_which')
        return '核对 DSHX 环境';
    if (name === 'dshx_check')
        return '检查插件合同';
    if (name === 'dshx_activation_plan')
        return '规划插件激活';
    if (name === 'dshx_sync_artifact')
        return '同步插件产物';
...
function subtitleOfTool(name, args, target) {
    if (target !== null)
        return target;
    if (name === 'bash')
        return commandOf(args) || name;
    const query = stringArg(args, 'query', 'pattern');
    return query ?? name;
}
function validReadPresentation(value) {
    if (!isRecord(value) || value.card !== 'read' || !Array.isArray(value.lines))
        return null;
    const lines = [];
    for (const line of value.lines) {
        if (!isRecord(line) || typeof line.number !== 'number' || typeof line.text !== 'string')
            return null;
...
function validReadMeta(value) {
    if (!isRecord(value) || !Array.isArray(value.lines) || typeof value.path !== 'string')
        return null;
    const lines = [];
    for (const line of value.lines) {
        if (!isRecord(line) || typeof line.number !== 'number' || typeof line.text !== 'string')
...
function validDiffs(value) {
    ...
    for (const diff of value) {
        if (!isRecord(diff) || typeof diff.path !== 'string' || typeof diff.newText !== 'string')
            return null;
...
function presentationOf(pair, rawText, exitCode, signal) {
    ...
    if (argDiff !== null)
        return argDiff;
    if (result !== null && result.card !== 'generic')
        return { kind: 'json', data: result };
    if (isRecord(pair.meta) || Array.isArray(pair.meta))
        return { kind: 'json', data: pair.meta };
...
function recognizedResultView(value) {
    if (!isRecord(value) || typeof value.card !== 'string')
        return false;
...
function statusOfTool(pair, exitCode, signal) {
    if (pair.result === null)
        return 'running';
    if (errorFlagOf(pair.result) || signal !== null || (exitCode !== null && exitCode !== 0))
        return 'failure';
    if (exitCode === 0)
        return 'success';
    ...
    if (domain !== null)
        return domain;
    if (recognizedResultView(pair.resultView))
        return 'success';
    if (pair.orphan && pair.name === '')
        return 'unknown';
    return 'returned';
}
...
function withModelPlaceholders(items, traces) {
    ...
    for (const trace of traces) {
        if (!occupied.has(`${trace.turn}:${trace.step}`))
            next.push(modelItem(trace));
    }
    return next;
}
function coordinatesOfLocation(value) {
    ...
    if (value.kind === 'step') {
        ...
        if (typeof turn?.turn === 'number' && typeof step?.step === 'number')
            return { turn: turn.turn, step: step.step };
    }
    if (value.kind === 'turn') {
        const turn = recordAt(value, 'turn');
        if (typeof turn?.turn === 'number')
            return { turn: turn.turn, step: 0 };
    }
    return null;
}
    ...
    for (const turnNumber of snapshot.chat.timeline.turnOrder) {
        ...
        for (const step of turn.steps) {
            for (const key of snapshot.chat.locations.getStep(turnNumber, step.step)) {
                ...
                if (!locations.has(node.anchorSeq))
                    locations.set(node.anchorSeq, coordinates);
                ...
                if (resultSeq !== null && !locations.has(resultSeq))
                    locations.set(resultSeq, coordinates);
            }
        }
    }
    return locations;
...
function trajectoryNodes(snapshot) {
    const views = snapshot.views;
    const trajectory = views.get('trajectory');
    if (isRecord(trajectory) && Array.isArray(trajectory.eventNodes))
        return trajectory.eventNodes;
...
export function pairsFromSnapshot(snapshot) {
...
function snapshotItems(snapshot) {
    ...
    for (const node of trajectoryNodes(snapshot)) {
        ...
        if (node.kind === 'user') {
            if (stringAt(node.source, 'kind') !== 'user')
                continue;
            const text = contentText(node.content);
            if (text !== '')
                items.push(messageItem(node, coordinates, 'user', text));
        }
        else if (node.kind === 'steering') {
            const text = contentText(node.content);
            if (text !== '')
                items.push(messageItem(node, coordinates, 'steering', text));
        }
            ...
            if (!hasTool && text.trim() !== '')
                items.push(messageItem(node, coordinates, 'assistant', text));
            items.push(...imageItems(node, coordinates));
            if (node.interrupted === true)
                items.push(turnStateItem(node.seq + 0.9, node.time, coordinates, 'interrupted', 'Agent 输出在完成前停止'));
        }
    ...
    snapshot.pending.forEach((pending, index) => {
        ...
        items.push(interactionItem(pending.key, Number.MAX_SAFE_INTEGER - 100 + index, Date.now(), { turn: latestTurn, step: latestStep }, 'waiting', name, null, pending));
    });
    const models = snapshot.chat.timeline.turnOrder.flatMap(turn => (snapshot.chat.timeline.turns.get(turn)?.steps.flatMap(step => {
...
function eventCoordinates(events) {
    const map = new Map();
    let turn = 0;
    let step = 0;
    for (const event of events) {
        ...
        if (event.seq !== undefined)
            map.set(event.seq, { turn: directTurn ?? turn, step: directStep ?? step });
    }
    return map;
}
function eventTimingIndex(events) {
    const turns = new Map();
    const steps = new Map();
    for (const event of events) {
        const data = isRecord(event.data) ? event.data : {};
...
function logMessageItems(events, coordinates) {
    const items = [];
    for (const event of events) {
        ...
        if (event.type === 'user/message') {
            ...
            if (stringAt(source, 'kind') !== 'user')
                continue;
            const text = contentText(data.content);
            if (text !== '') {
                const node = { kind: 'user', seq, time: event.time ?? 0, content: [], source: null };
            ...
            for (const inserted of data.inserted) {
                if (!isRecord(inserted) || stringAt(inserted.source, 'kind') !== 'user')
                    continue;
                const text = contentText(inserted.content);
                if (text === '')
                    continue;
                const node = { kind: 'steering', seq, time: event.time ?? 0, content: [], source: null };
            ...
            if (!hasTool && text !== '') {
                const node = { kind: 'assistant', seq, time: event.time ?? 0, turn: location.turn, step: location.step, blocks: [] };
            ...
            for (const block of content) {
                ...
                const base = messageItem({ kind: 'assistant', seq, time: event.time ?? 0, turn: location.turn, step: location.step, blocks: [] }, location, 'assistant', '');
                items.push({
                    ...base,
            ...
            if (kind === 'aborted')
                items.push(turnStateItem(seq, event.time ?? 0, location, 'interrupted', '用户或运行时中断了本轮对话'));
            else if (kind !== null && kind !== 'completed')
                items.push(turnStateItem(seq, event.time ?? 0, location, 'failure', safeStringify(reason)));
        }
    }
    return items;
}
function approvalItems(events, coordinates) {
    ...
    for (const event of events) {
        ...
        if (event.type === 'approval/decided') {
            ...
            if (id !== null)
                decided.set(id, event);
        }
        else if (event.type === 'tool/call') {
            const callId = stringAt(data, 'callId');
            if (callId !== null)
                callLocations.set(callId, coordinates.get(event.seq ?? 0) ?? { turn: 0, step: 0 });
        }
    }
    const items = [];
    for (const event of events) {
        ...
        const status = decision === undefined
            ? 'waiting'
...
function withPatterns(items) {
    ...
    for (let index = 0; index < next.length; index++) {
        ...
        if (item.signature !== null) {
            ...
            if (item.status === 'success' && previous !== undefined && previous.status === 'failure')
                previous.recoveredBy = item.id;
            lastBySignature.set(item.signature, index);
        }
        if (item.intentKey !== null) {
            const previousIndex = lastByIntent.get(item.intentKey);
            const previous = previousIndex === undefined ? undefined : next[previousIndex];
            if (previous !== undefined && previous.signature !== item.signature)
                item.iterationIndex = previous.iterationIndex + 1;
...
function statusOfItems(items) {
    ...
    if (items.some(item => item.status === 'interrupted'))
        return 'interrupted';
    if (items.some(item => item.status === 'failure' && item.recoveredBy === null))
        return 'failure';
...
function stepOf(items, timing, model) {
    const first = items[0];
    if (first === undefined)
        throw new Error('work step requires at least one item');
...
function stepsOf(items, timingOf, modelOf) {
    ...
    for (const item of items) {
        ...
        if (current.length > 0 && itemKey !== key) {
            ...
            if (first !== undefined) {
                steps.push(stepOf(current, timingOf?.(first.turn, first.step) ?? { startTime: null, endTime: null }, modelOf?.(first.turn, first.step) ?? null));
            }
            current = [];
        }
        key = itemKey;
...
function groupOf(steps) {
    const first = steps[0];
    if (first === undefined)
        throw new Error('work group requires at least one step');
...
function pictureOf(sourceItems, options) {
    if (sourceItems.length === 0)
        return { ...EMPTY_PICTURE, running: options.running, partialHistory: options.partialHistory, pendingCount: options.pendingCount };
    const occupiedSteps = new Set(sourceItems
        .filter(item => item.source !== 'model')
        .map(item => `${item.turn}:${item.step}`));
    const visibleSourceItems = sourceItems.filter(item => (item.source !== 'model' || !occupiedSteps.has(`${item.turn}:${item.step}`)));
    ...
    const items = withPatterns([...visibleSourceItems].sort((left, right) => left.turn - right.turn
...
export function foldSnapshot(snapshot, options = {}) {
    if (snapshot.blank) {
        return {
            ...EMPTY_PICTURE,
 ...
 */
export function mergeObservedPictures(previous, current) {
    ...
    const items = new Map();
    for (const item of itemsOfPicture(previous))
        items.set(item.id, item);
    for (const item of itemsOfPicture(current))
        items.set(item.id, item);
    ...
    for (const [key, model] of stepModelsOfPicture(current))
...
export function foldEvents(events, options = {}) {
    ...
    const items = withModelPlaceholders([...pairTools(events).map(toolItem), ...logMessageItems(events, coordinates), ...approvalItems(events, coordinates)], models.values());
