...
try {
    switch ($Task) {
        'fmt' {
            moon fmt
            if ($LASTEXITCODE -ne 0) { throw "moon fmt 失败" }
        }
        'check' {
            moon check --deny-warn --target native
            if ($LASTEXITCODE -ne 0) { throw "moon check 失败" }
        }
        'test' {
            moon test --target native
            if ($LASTEXITCODE -ne 0) { throw "moon test 失败" }
        }
        'build' {
            moon build --target native --release
            if ($LASTEXITCODE -ne 0) { throw "moon build 失败" }
